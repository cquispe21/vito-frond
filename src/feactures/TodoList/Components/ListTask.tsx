...
export default function ListTask() {
  const { tasks, searchTask, setTransition, transition } = useContext(
    TodoContext
  ) as ITodoContext;
  useEffect(() => {
    setTimeout(() => {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-1 gap-2 my-2 mx-3 cursor-pointer">
          {tasks.map((task) => (
            <div
              ...
              className={` ${
                task.estado == "completada" && "bg-[#A7F3D0]"
              } bg-white shadow-md rounded-lg p-2 border border-gray-200 hover:shadow-lg transition-all`}
            >
